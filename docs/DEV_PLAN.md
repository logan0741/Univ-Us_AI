...
GitHub 저장소 → **Projects** → 새 프로젝트 → **Board** 로 만듭니다.

```mermaid
flowchart LR
    A["📥 Backlog<br/>등록만 됨"] --> B["🎯 Ready<br/>완료 기준 확정"]
    B --> C["🔨 In Progress<br/>1인 최대 2장"]
    C --> D["👀 Review<br/>PR 올라감"]
    D --> E["✅ Done<br/>기능 메인 브랜치 머지"]
    D -.수정 요청.-> C
```

| 컬럼 | 여기 있으려면 |
| --- | --- |
| **Backlog** | 이슈로 등록만 된 상태. 아직 안 해도 됨 |
| **Ready** | **완료 기준이 적혀 있어야** 넘어옵니다. "무엇이 되면 끝인지" 모르면 시작하지 않습니다 |
| **In Progress** | 실제로 코드를 쓰는 중. **1인 동시 2장까지** |
| **Review** | PR을 올리고 리뷰어를 지정한 상태 |
...
2. **In Progress 2장 제한.** 벌려놓기만 하고 끝나는 게 없는 상태를 막습니다.
   3장째를 시작하고 싶으면, 먼저 하나를 Review로 보내세요.
3. **막히면 `blocked` 라벨.** 컬럼을 옮기지 말고 라벨을 답니다. 하루 이상 혼자 붙잡지 않습니다.
4. **Done의 기준은 "머지"입니다.** "코드는 다 짰는데 PR은 아직"은 여전히 In Progress입니다.
